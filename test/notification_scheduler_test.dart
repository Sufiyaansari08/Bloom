...
void main() {
  ...
  group('NotificationService Milestone Alerts and IDs', () {
    ...
    test('schedulePeriodAlert schedules advance and day-of alerts with correct texts', () async {
      ...
      final periodAlert = fakePlatform.scheduled.firstWhere((e) => e['id'] == 102);
      expect(periodAlert['title'], 'Expected Period Alert');
      expect(periodAlert['body'], 'Period expected in 2 days.');

      final todayAlert = fakePlatform.scheduled.firstWhere((e) => e['id'] == 106);
      expect(todayAlert['title'], 'Expected Period Alert');
      expect(todayAlert['body'], 'Period expected today.');
    });

    test('schedulePeriodAlert handles 1 day before text and discrete mode', () async {
      final now = DateTime.now();
      ...
      expect(periodAlert['body'], 'You have an upcoming cycle milestone tomorrow.');

      final todayAlert = fakePlatform.scheduled.firstWhere((e) => e['id'] == 106);
      expect(todayAlert['title'], 'Cycle Reminder 🌸');
      expect(todayAlert['body'], 'You have an expected cycle milestone today. Tap to view.');
    });

    test('scheduleFertileAlert handles eve and start day in standard and discrete modes', () async {
      final now = DateTime.now();
      ...
      final eveAlert = fakePlatform.scheduled.firstWhere((e) => e['id'] == 103);
      expect(eveAlert['title'], 'Fertile Window Alert');
      expect(eveAlert['body'], 'Fertile window is expected to start tomorrow.');

      final startAlert = fakePlatform.scheduled.firstWhere((e) => e['id'] == 107);
      expect(startAlert['title'], 'Fertile Window Alert');
      expect(startAlert['body'], 'Fertile window is expected to start today.');
      ...
      final discreteEve = fakePlatform.scheduled.firstWhere((e) => e['id'] == 103);
      expect(discreteEve['title'], 'Wellness Update ✨');
      expect(discreteEve['body'], 'A new cycle phase begins tomorrow. Tap to view in Bloom.');

      final discreteStart = fakePlatform.scheduled.firstWhere((e) => e['id'] == 107);
      expect(discreteStart['title'], 'Wellness Update ✨');
      expect(discreteStart['body'], 'A new phase update is ready in Bloom. Tap to check your insights.');
    });

    test('scheduleOvulationAlert handles eve and day-of in standard and discrete modes', () async {
      final now = DateTime.now();
      ...
      final eveAlert = fakePlatform.scheduled.firstWhere((e) => e['id'] == 108);
      expect(eveAlert['title'], 'Ovulation Alert');
      expect(eveAlert['body'], 'Ovulation expected tomorrow.');

      final dayAlert = fakePlatform.scheduled.firstWhere((e) => e['id'] == 104);
      ...
      final discreteEve = fakePlatform.scheduled.firstWhere((e) => e['id'] == 108);
      expect(discreteEve['title'], 'Health & Cycle Tip 🌸');
      expect(discreteEve['body'], 'An important cycle milestone is predicted for tomorrow.');

      final discreteDay = fakePlatform.scheduled.firstWhere((e) => e['id'] == 104);
      expect(discreteDay['title'], 'Health & Cycle Tip 🌸');
      expect(discreteDay['body'], 'New daily insight ready for you in Bloom.');
    });

    test('scheduleDailyCheckIn schedules daily routine at custom time in standard and discrete modes', () async {
      ...
      final checkIn = fakePlatform.scheduled.firstWhere((e) => e['id'] == 101);
      expect(checkIn['title'], 'Daily Check-in Reminder');
      expect(checkIn['body'], 'Remember to log your mood, flow, and symptoms for today.');
      ...
      final discreteCheckIn = fakePlatform.scheduled.firstWhere((e) => e['id'] == 101);
      expect(discreteCheckIn['title'], 'Daily Check-in 🌿');
      expect(discreteCheckIn['body'], 'Time for your daily Bloom check-in.');
    });

    test('Same-day delivery triggers instant notification when active on milestone day', () async {
      final now = DateTime.now();
      ...
      // Should either be scheduled for 9:00 AM (if test runs before 9 AM) or instant (if after 9 AM)
      final hasInstant = fakePlatform.instant.any((e) => e['id'] == 104);
      final hasScheduled = fakePlatform.scheduled.any((e) => e['id'] == 104);
      expect(hasInstant || hasScheduled, isTrue);

      if (hasInstant) {
        final alert = fakePlatform.instant.firstWhere((e) => e['id'] == 104);
        expect(alert['title'], 'Ovulation Day Alert');
        expect(alert['body'], 'Ovulation is expected today.');
      }
    });
    ...
    test('showCycleSummaryNotification sends instant alert in standard and discrete modes', () async {
      // Standard mode
      await NotificationService.instance.showCycleSummaryNotification(isDiscrete: false);
      final alert = fakePlatform.instant.firstWhere((e) => e['id'] == 105);
      expect(alert['title'], 'Monthly Cycle Summary');
      expect(alert['body'], 'Your cycle summary and insights are ready. Tap to view your cycle trends.');
      expect(alert['payload'], '/insights');

      // Discrete mode
      fakePlatform.instant.clear();
      await NotificationService.instance.showCycleSummaryNotification(isDiscrete: true);
      final discreteAlert = fakePlatform.instant.firstWhere((e) => e['id'] == 105);
      expect(discreteAlert['title'], 'Wellness Insights 🌿');
      expect(discreteAlert['body'], 'Your latest wellness summary is ready in Bloom.');
      expect(discreteAlert['payload'], '/insights');
    });

    test('scheduleMonthlyCycleSummary schedules recurring monthly alert', () async {
      ...
      final scheduledAlert = fakePlatform.scheduled.firstWhere((e) => e['id'] == 105);
      expect(scheduledAlert['title'], 'Monthly Cycle Summary');
      expect(scheduledAlert['id'], 105);
    });

    test('Cycle notification cache prevents duplicate alerts for the same cycle', () {
